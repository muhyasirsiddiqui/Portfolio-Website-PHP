...
<body>
  ...
  <!----------****Edite Action Start****------------->
					 ...
					 if(isset($_GET["edit_id"]))
					 {
						 ...
						 $query = mysqli_query($connection,"select * from services where Id='".$editcakeid."'");
...
if(!empty($_POST["hideid"]))
                {
  if (empty($_FILES["fileupload1"]["name"]) && empty($_FILES["fileupload2"]["name"]))
                        {
$mypath = $_POST["path"];
...
if(empty($_POST["hideid"]))
                        ...
                        if (!empty($_FILES["fileupload1"]["name"])&& !empty($_FILES["fileupload2"]["name"]))
                        {
                          if (move_uploaded_file($temp_name1,$Location.$Filename1))
                          {
                            if (move_uploaded_file($temp_name2,$Location.$Filename2))
                          {
  $query = "insert into services (Name,Description,Summary,Img_URL_1,Img_URL_2,Is_Active,Created_On,Updated_On,Created_By) 
  values('".$service_name."','".$service_des."','".$service_summary."'
  ,'".$Filename1."','".$Filename2."','".$active_chk."',CURRENT_DATE,
CURRENT_DATE,'admin')";
...
<!---Insert Action End--->
					...
					<div class="col-lg-12">
                    <div class="card">
                    <form method="post" action="ServiceForm.php" enctype="multipart/form-data">
